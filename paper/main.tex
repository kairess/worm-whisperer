...
\caption{\textbf{Chemotaxis through command and steering interneurons.} (A) Learning curves under the curriculum. (B) Trajectories of the trained policy. (C) Policy sweep: stimulation of each channel as a function of the observed concentration change. (D) Reversal-onset rate by $\mathrm{d}C/\mathrm{d}t$ quintile compared with Pierce-Shimomura et al.\ \cite{r9}.}
\label{fig2}
\end{figure}
...
\caption{\textbf{The minimal circuit.} Reach rate after masking each channel of the trained policy, and after retraining without RIV or without steering.}
\label{fig3}
\end{figure}
...
\caption{\textbf{Mazes.} Recorded trajectories in the L-corridor, T-maze and grid, with the corner sequence stall $\to$ reversal $\to$ deep bend.}
\label{fig4}
\end{figure}
...
\caption{\textbf{Bend direction is turn direction.} Left- and right-turn corridors under the ventral-only and SMD-direction rules across corridor widths, and the dorsal-swap control.}
\label{fig5}
\end{figure}
