...
| 질문 | 판정 | 핵심 수치 |
|---|---|---|
| 명령·조향 개재뉴런(AVB, AVA, SMDD, SMDV, RIV)만 자극해 화학주성이 되는가? | ✓ (5 시드 중 4) | 2.5 mm, 40초 안 도달률 0.914 (무작위 0.04). 시드 0–4: 0.914 / 0.973 / 0.301 / 0.969 / 0.934 (평균 0.82 ± 0.29); 시드 2 는 후진 규칙은 배웠지만 깊은 굽힘을 쓰지 않았다 |
...
| 명령 뉴런을 직접 자극해도 실제 배선이 중요한가? | ✓ | 배선을 뒤섞은 커넥톰(가중치·개수·부호 보존): 재학습 0.14 / 0.15 / 0.32 (3 시드), 전이 0.02–0.06 vs 실제 배선 0.871 — 뒤섞인 배선에서는 깊은 굽힘 펄스가 거의 안 난다. **층별 해리:** 화학 시냅스만 뒤섞으면 그대로(재학습 0.859, 전이 0.914), 갭정션만 뒤섞으면 붕괴(0.078 / 0.125). 이 적합 동역학에서 자극은 갭정션 층을 통해 판독 뉴런에 닿는다 |
| 몸 폭 4배 통로에서 모퉁이는 어떻게 도는가? | | 벽에 눌린 깊은 굽힘으로만; 머리 조향만으로는 불가(고정 시작 시험). **굽힘의 등/배 방향이 곧 회전 방향** |
| 배쪽 굽힘만 있으면(기본 오메가턴)? | | T-미로 첫 진입 ≥ 99 % 왼쪽 팔; 오른쪽 통로 0–6 %(흔들린 시작 32개); 3×3 격자 불가 |
| 실제 벌레처럼 SMDD/SMDV 가 굽힘 방향을 정하게 하면? | ✓ (ADR-017) | 오른쪽 통로를 배운다(흔들린 시작의 0.97–1.00) — 단 정책은 한손잡이가 된다; 통로 폭 0.25–0.35 mm 에서 강건 |
| T 갈림길에서 맞는 팔을 고를 수 있는가? | | 시간 감각만으로는 원리상 불가(갈림길에서 두 팔의 냄새가 같음) → 학습된 전략은 "왼쪽 먼저, 나빠지면 후진" (도달 1.00, 오른쪽 목표는 5–10초 손해). 좌우 농도차를 주면 굽힘 방향이 소스 쪽을 따른다(펄스의 73 %; 없으면 62 %) |
...
핵심 도달률과 팔 선택은 미세 적분 스텝(dt 0.05 ms, float64)에서 같은 시작점으로 재확인했고 변하지 않았다; 미로 통과 시간은 미세 스텝에서 25 % 길어진다(`docs/PAPER_OUTLINE.md` §7 E8). 학습은 GPU 배치 시뮬레이터(`worm/env/batch.py`, 롤아웃당 수백 마리, B=256 에서 초당 약 6 에피소드) 위의 OpenAI-ES. 평가는 항상 사전 등록된 규격으로 새 무작위 시작 128–256회.
