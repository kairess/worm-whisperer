...
| 학습이 피루엣 규칙을 스스로 찾는가? | ✓ (규칙) / ✗ (충분성) | 농도 하강 → 후진+오메가, 상승 → 전진 (ρ −1.00); 후진 개시율은 Pierce-Shimomura 1999 처럼 dC/dt 에 따라 단조 감소; 그러나 후진+오메가만으로는 항법 불가(0.05) |
| 최소 회로는? | | 후진(AVA) + 깊은 굽힘(RIV) + 조향(SMD) 셋 모두: 하나만 절제해도 0.07–0.27, 하나 빼고 재학습해도 실패 |
...
| 실제 벌레처럼 SMDD/SMDV 가 굽힘 방향을 정하게 하면? | ✓ (ADR-017) | 오른쪽 통로를 배운다(1.00) — 단 정책은 한손잡이가 된다 |
...
1부 2026-09-03 마감; Worm Gym 과 미로 항법 2026-09-03…06. 투고 전 실험(시드 반복, 폭 민감도, 배선 뒤섞기 대조군)이 진행 중이며 `docs/PAPER_OUTLINE.md` 3절에 기록된다. Claude Code 와 함께 개발했고 세션 원본은 `history/`.
