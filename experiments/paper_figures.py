...
OUT = "docs/figures"; os.makedirs(OUT, exist_ok=True); plt.rcParams.update({"font.size": 10, "axes.spines.top": False, "axes.spines.right": False, "font.family": "serif", "font.serif": ["Times New Roman", "Nimbus Roman", "Liberation Serif", "DejaVu Serif"], "mathtext.fontset": "stix", "savefig.dpi": 300})
...
ax[0].set_xlabel("generation"); ax[0].set_ylabel("reach rate (training seeds)"); ax[0].legend(frameon=False); ax[0].set_title("A  learning (command/steering channels)")
...
if sw:
    ...
    ax[1].axvline(0, color="#999", lw=0.8); ax[1].set_xlabel("10·Δlog C per 0.5 s (falling ← → rising)"); ax[1].set_ylabel("stimulus (fraction of 6 pA)"); ax[1].legend(frameon=False, ncol=5, fontsize=8, loc="upper center", bbox_to_anchor=(0.5, -0.3), handlelength=1.2, columnspacing=0.8); ax[1].set_title("B  policy sweep: input → stimulation")
...
ax[2].set_xlabel("dlogC/dt quintile (1 = falling)"); ax[2].set_ylabel("reversal onset rate (/s)"); ax[2].set_title("C  reversal onsets vs dC/dt")
...
ax[0].bar(list(abl), list(abl.values()), color=["#555"] + ["#c44e52"] * 6); ax[0].set_ylabel("reach rate (2.5 mm, 40 s, n=256)"); ax[0].set_title("A  channel ablation of the trained policy"); plt.setp(ax[0].get_xticklabels(), rotation=30, ha="right")
...
for ax, (sid, title) in zip(axes, scenes):
    ...
    ax.add_patch(plt.Circle(d["src"], d["reach_r"], fill=False, ls="--", color="#55a868")); ax.set_aspect("equal"); ax.set_title(f"{title}\nreached {d['reached']}, t = {F[-1]['t']:.0f} s", fontsize=10, pad=6); ax.set_xticks([]); ax.set_yticks([])
axes[0].legend(frameon=False, fontsize=8, loc="upper left"); fig.tight_layout(rect=(0, 0, 1, 0.92)); fig.savefig(f"{OUT}/fig4_mazes.png", dpi=300); plt.close(fig)
...
ax[1].set_xticks(xx); ax[1].set_xticklabels(labels, fontsize=9); ax[1].set_ylim(0, 1.25); ax[1].legend(frameon=False, fontsize=8, loc="upper left", ncol=2); ax[1].set_ylabel("reach rate (2.5 mm, n=256)"); ax[1].set_title("B  wiring shuffles: gap junctions carry the stimulus")
...
for x, (title, body, col) in zip(xs, boxes):
    ...
    ax.text(x + W / 2, 2.62, title, ha="center", va="center", fontsize=10.5, fontweight="bold"); ax.text(x + W / 2, 1.55, body, ha="center", va="center", fontsize=8.8)
...
ax.text(6.3, -0.55, "next observation every 0.5 s: odour at the nose, wall contact", ha="center", fontsize=9.5, color="#444")
ax.text(6.2, 3.3, "Whitelist principle: the agent never touches motor neurons or muscles; the sensory-to-command step is left to the wiring", ha="center", fontsize=10.5, fontweight="bold")
